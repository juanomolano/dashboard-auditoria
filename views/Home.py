...
def render_home():
    ...
    # ---------------------------------------------------------
    # NUEVA SECCIÓN: PORCENTAJE DE PARTICIPACIÓN / HALLAZGOS POR REGIONAL
    # ---------------------------------------------------------
    st.markdown("---")
    st.subheader("📊 Distribución y Porcentaje Consolidado por Regional")

    if not df_reg_all.empty:
        col_reg_t, col_reg_g = st.columns([1, 1])

        # Cálculos por Regional
        df_reg_counts = df_reg_all["REGIONAL"].value_counts().reset_index()
        df_reg_counts.columns = ["Regional", "Cantidad de Hallazgos"]
        
        # Calcular porcentaje respecto al total
        df_reg_counts["Porcentaje Participación (%)"] = (df_reg_counts["Cantidad de Hallazgos"] / total_inconsistencias * 100).round(2)
        
        # Calcular Porcentaje de Cumplimiento Inverso (si aplica)
        df_reg_counts["Porcentaje Formateado"] = df_reg_counts["Porcentaje Participación (%)"].astype(str) + "%"

        with col_reg_t:
            st.markdown("##### 📋 Resumen Tabular por Regional")
            st.dataframe(
                df_reg_counts[["Regional", "Cantidad de Hallazgos", "Porcentaje Formateado"]],
